...
int main(int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, "f:t:hvVgal")) != EOF) {
        ...
        case 't': /* Directory where the traces are located */
            ...
            if (tracedir[strlen(tracedir) - 1] != '/') strcat(tracedir, "/"); /* path always ends with "/" */
            break;
...
static trace_t *read_trace(char *tracedir, char *filename) {
    ...
    if ((trace = (trace_t *)malloc(sizeof(trace_t))) == NULL) unix_error("malloc 1 failed in read_trance");
...
static double eval_mm_util(trace_t *trace, int tracenum, range_t **ranges) {
    ...
    for (i = 0; i < trace->num_ops; i++) {
        ...
        case REALLOC: /* mm_realloc */
            ...
            oldp = trace->blocks[index];
            if ((newp = mm_realloc(oldp, newsize)) == NULL) app_error("mm_realloc failed in eval_mm_util");
...
static void eval_mm_speed(void *ptr) {
    ...
    for (i = 0; i < trace->num_ops; i++) switch (trace->ops[i].type) {
        ...
        case REALLOC: /* mm_realloc */
            ...
            newsize = trace->ops[i].size;
            oldp = trace->blocks[index];
            if ((newp = mm_realloc(oldp, newsize)) == NULL) app_error("mm_realloc error in eval_mm_speed");
            trace->blocks[index] = newp;
            break;
...
static void eval_libc_speed(void *ptr) {
    ...
    for (i = 0; i < trace->num_ops; i++) {
        ...
        case REALLOC: /* realloc */
            ...
            newsize = trace->ops[i].size;
            oldp = trace->blocks[index];
            if ((newp = realloc(oldp, newsize)) == NULL) unix_error("realloc failed in eval_libc_speed\n");
